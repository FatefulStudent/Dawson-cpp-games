...
public:
    ...
    Card(rank cRank = ACE, suit cSuit = CLUBS, bool ifu = true);
...
Card::Card(rank cRank, suit cSuit, bool ifu):
m_Rank(cRank),
m_Suit(cSuit),
m_FaceIsUp(ifu)
{}
...
void Card::Flip()
{
    m_FaceIsUp = !m_FaceIsUp;
}

class Hand  //abstract class
{
public:
    Hand();
    virtual ~Hand();
...
public:
    ...
    // returns whether the player is busted or not (has greater than 21 points)
    bool IsBusted() const;

    // announces that generic player is busted
    void Bust() const;
...
void GenericPlayer::AddToHand(Card* pCard)
{
    m_Hand.Add(pCard);
}
...
public:
    ...
    //announces that the player draws with house
    void Push() const;
};
...
void Deck::Deal(GenericPlayer& aGenericPlayer)
{
    if (!m_Cards.empty())
    {
        aGenericPlayer.AddToHand(m_Cards.back());
        m_Cards.pop_back();
    }
    else
    {
...
void Game::Play()
{
    ...
    // sum up the results and clear everything
    OnRoundEnd();
}
...
std::ostream& operator<<(std::ostream& os, const GenericPlayer& aGenericPlayer)
{
    ...
    if (!aGenericPlayer.m_Hand.IsEmpty())
    {
        for (pCard = aGenericPlayer.m_Hand.begin(); 
             pCard != aGenericPlayer.m_Hand.end(); 
             ++pCard)
        {
            os << *(*pCard) << "\t";
        }
